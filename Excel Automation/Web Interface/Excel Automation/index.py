...
def index():
    global flag
    return render_template('index.html', flag=flag, error="no")
...
def output():
    ...
    if flag == 0:
        ...
        cust_file_path = f"upload_folder/{secure_filename(customer_bom.filename)}"
        customer_bom.save(cust_file_path)
        manex_bom = request.files['manex_bom']
        man_file_path = f"upload_folder/{secure_filename(manex_bom.filename)}"
        ...
        CustomerBom = customer_bom.filename
        ManexBom = manex_bom.filename
        flag = 0
        if safe:
            return render_template('output.html', customer_bom=customer_bom.filename, manex_bom=manex_bom.filename)
        else:
            return render_template('index.html', flag=flag, error="notsafe")
    else:
