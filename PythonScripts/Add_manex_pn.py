...
def MapDes(customer_bom, manex_bom, bom_data, file_extension, start_row, end_row, bom_col_des, sep_dict, sep_position):
    ...
    result, error = ManexInfo(manex_bom, sep_dict)
    if not bool(result):
        return False, error
    manex_data = result[0]
    manex_start_row = result[1]
    manex_end_row = result[2]
    ws_manex = result[3]
    wb_manex = result[4]
    manex_col_partno = result[5]
    ...
    try:
        ...
        man, error_man = WriteManexBom(manex_bom, manex_start_row, manex_end_row, ws_manex, wb_manex, manex_col_partno, manex_pn, duplicate)
        if cust is not True:
    ...
    try:
        ...
        if file_extension == ".xls" or file_extension == ".XLS":
            ...
            if separator is not None:
                ...
                return [sep_data, bom_data, file_extension, bom_col_des, sep_position], None
            else:
                for row in range(start_row-1, end_row):
                    var = ws_bom.row_values(row)
                    x = var[bom_col_des]
                ...
                result, error = MapDes(customer_bom, manex_bom, bom_data, file_extension, start_row, end_row, bom_col_des, {}, [])
...
def CustBomInfo(sep_detail, bom_data, customer_bom, manex_bom, file_extension, start_row, end_row, bom_col_des, sep_dict, sep_position):
    ...
    logging.info("Finished reading")
    result, error = MapDes(customer_bom, manex_bom, bom_data, file_extension, start_row, end_row, bom_col_des, sep_dict, sep_position)
    if result is not True:
        return False, error
    return True, None
...
def ManexInfo(manex_bom, sep_dict):
    ...
    try:
        ...
        logging.info("Finished reading")
        return [manex_data, manex_start_row, manex_end_row, ws_manex, wb_manex, manex_col_partno], None
